...
def explain_binary_search_tree():
    ...
    clear_console()
    input("Enter for next info")
    logger.info(
        "Бинарное дерево поиска (BST) — это бинарное дерево, в котором для каждого узла значения в левом поддереве меньше, чем значение узла, а значения в правом поддереве больше. Это позволяет эффективно искать элементы с временной сложностью O(log n) в сбалансированных деревьях."
    )

    logger.info(
        "Бинарное дерево — это более общее понятие, которое не имеет ограничений на размещение значений в узлах. В нём каждый узел может иметь до двух дочерних узлов (левого и правого), но порядок их размещения не определён."
    )

    logger.info(
        "Сбалансированное дерево — это бинарное дерево поиска, в котором высота левого и правого поддеревьев каждого узла отличается не более чем на 1. Это обеспечивает эффективность операций поиска, добавления и удаления с временной сложностью O(log n). Примеры: AVL-деревья, красно-черные деревья."
    )

    logger.info(
        "Несбалансированное дерево — это бинарное дерево, где высота левого и правого поддеревьев может сильно различаться. Это может привести к ухудшению производительности операций поиска, добавления и удаления до O(n) в худшем случае, если дерево становится линейным, как список."
    )
...
def explain_graph():
    ...
    logger.info(
        "Графы делятся на несколько типов:\n"
        "1. Ориентированные (направленные) графы: рёбра имеют направление.\n"
        "2. Неориентированные графы: рёбра не имеют направления.\n"
        "3. Взвешенные графы: рёбра имеют вес (стоимость перехода).\n"
        "4. Невзвешенные графы: рёбра не имеют весов.\n"
        "5. Связные графы: из любой вершины можно попасть в любую другую.\n"
        "6. Несвязные графы: существуют изолированные компоненты.\n"
        "7. Ациклические графы: графы, не содержащие циклов.\n"
        "8. Циклические графы: графы, содержащие хотя бы один цикл."
    )
